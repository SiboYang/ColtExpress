...
from assets.media_constants import LOCOMOTIVE
...
class Locomotive(pygame.sprite.Sprite, Components):
    ...
    def __init__(self,
        ...
        self.image = pygame.image.load(LOCOMOTIVE).convert_alpha()
    ...
    def chosen(self, bandit: Bandit, event):
    ...
    def chosen(self, bandit: Bandit, event, index, roof):
        if event.type == pygame.MOUSEBUTTONUP:
            mouse = pygame.mouse.get_pos()
            # when clicking the upper part of the wagon  (y * 0.3)
            if self.x < mouse[0] < self.x + self.width and self.y < mouse[1] < self.y + self.height * 0.5:
